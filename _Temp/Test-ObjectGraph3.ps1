...
# https://www.newtonsoft.com/json/help/html/Properties_T_Newtonsoft_Json_Schema_JsonSchema.htm
    ...
    function ResolveReferences($Node) {
        ...
        while ($true) {
            ...
            $RefNode = if ($Node.Contains('@References')) { $Node.GetChildNode('@References') }
    ...
    function TestNode (
        ...
        $At = [Ordered]@{} # $At{<Assert Test name>] = $ChildNodes.@<Assert Test name>
        ...
        if ($At.Contains('CaseSensitive')) { $CaseSensitive = [Nullable[Bool]]$At['CaseSensitive'] }
...
#Region Node validation
        ...
        foreach ($TestName in $Tests.Keys) {
            if ($TestName -notin $At.Keys) { continue } # Check if ordered test are still required !!!
            $AssertNode = $At[$TestName]
            ...
            elseif ($TestName -in 'Ordered', 'RequiredNodes', 'AllowExtraNodes') {
                if ($ObjectNode -isnot [PSCollectionNode]) {
                    $Violates = "The '$($AssertNode.Name)' assert requires a collection node"
                }
            }
...
#Region Required nodes
        ...
        $RequiredNodes = $At['RequiredNodes']
        $RequiredList = if ($RequiredNodes) { [List[Object]]$RequiredNodes.Value } else { [List[Object]]::new() }
        foreach ($ChildNode in $ChildNodes) {
            if ($ChildNode -is [PSCollectionNode] -and $ChildNode.GetValue('@Required')) { $RequiredNodes.Add($ChildNode.Name) }
        }
            ...
            While ($Stack.Count -gt 0) {
                ...
                while ($Compute -or $Enumerator.MoveNext()) {
                    ...
                    if ($null -ne $Operand) {
                        ...
                        if ($Operator -eq 'And') {
                            $Operator = $null
                            if ($Accumulator -eq $false -and -not $At['AllowExtraNodes']) { break }
                            $Accumulator = $Accumulator -and $Operand
                        }
                        elseif ($Operator -eq 'Or') {
                            $Operator = $null
                            if ($Accumulator -eq $true -and -not $At['AllowExtraNodes']) { break }
...
#Region Optional nodes
        ...
        if (-not $Violates) {
            ...
            foreach ($TestNode in $TestNodes) {
                ...
                MatchNode @MatchParams
                if ($At['AllowExtraNodes'] -and $MatchedNames.Count -eq $MatchCount0) {
            ...
            if (-not $At['AllowExtraNodes'] -and $MatchedNames.Count -lt $ChildNodes.Count) {
